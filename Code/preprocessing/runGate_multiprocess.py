...
def check_gate_ram_usage(proc):
    # Check the RAM usage of the GATE process
    try:
        proc_info = psutil.Process(proc.pid)
...
def check_gate_peak_usage(proc, wait_time=45, check_interval=5, stable_threshold=3):
    ...
    try:
        ...
        while proc.is_running():
            ...
            if stable_ram_usage == 0 or abs(ram_usage - stable_ram_usage) < (50 * 1024 ** 2):
...
def run_macro(macro_file, output_location):
    ...
    return {"file": macro_file.name, "status": "completed", "pid": proc.pid}


@timer
def manage_Gate_processes_p(output_location, cores, ram):
    try:
    ...
    try:
        ...
        # Sorting function: Extract numeric part of the file path and use it as the sorting key
    ...
    with ProcessPoolExecutor(max_workers=max_processes) as executor:
        try:
            # Submit `run_macro` and return only simple data
            ...
            for future in as_completed(future_to_macro):
                ...
                try:
                    ...
                    print(f"Macro file {macro_file} finished with PID {result['pid']}.")
